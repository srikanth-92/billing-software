...
function isInAppBrowser() {
  ...
  return (
    ...
    /WhatsApp/i.test(ua) ||
    /Paytm/i.test(ua) ||
    (/Android/i.test(ua) && /wv/.test(ua)) ||  // Android WebView
    (/iPhone|iPad|iPod/i.test(ua) && !/Version\/[\d.]+ .*Safari\//.test(ua) && !/CriOS|FxiOS|EdgiOS/.test(ua))  // iOS WebView
  );
}
...
export default function GuestPaymentScreen({ navigation, route }) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      console.log('[GuestPaymentScreen] Browser check:', { inAppBrowser, ua: navigator.userAgent });

      // ALWAYS use QR for web (simpler, works everywhere)
      if (Platform.OS === 'web') {
        console.log('[GuestPaymentScreen] Using QR code payment for web');
        try {
          ...
          console.error('[GuestPaymentScreen] QR creation failed:', err);
          if (cancelled) return;
          // Fallback to local UPI QR
          const upi = buildUpiString({ amountRupees: total, orderId, restaurantName: RESTAURANT_NAME });
          setQrUpiString(upi);
          setState(STATE.AWAITING);
        }
      } else {
        // Native platform - also use QR
        try {
          const { qrId: id, imageUrl } = await createRazorpayQR({
    ...
    <>
      ...
      {state === STATE.AWAITING && (
        <View style={styles.qrSection}>
          ...
          <Text style={styles.autoHint}>Screen updates automatically when payment is received</Text>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  autoHint: { fontSize: 11, color: THEME.slateLight, textAlign: 'center', paddingHorizontal: 30, lineHeight: 16, marginTop: 8 },
